...
- [x] **POST body from script** — `HttpSourceConnector` supports `body` field on `from.http` config: an inline expression or `.elwood` script reference evaluated against the IDM, serialized as the POST/PUT request body
- [x] **Accepted status codes** — `HttpSourceConnector` supports `acceptedStatusCodes` field (e.g., `"2xx,4xx,5xx"`): captures the response on non-2xx instead of throwing. Status code exposed via `$source.http.statusCode` in source map scripts
- [x] **Dynamic response status code** — `OutputConfig` supports `responseStatusCode` field: expression evaluated against the IDM that sets the HTTP response status code (e.g., `$.crmStatusCode`). `PipelineResult` carries the resolved status code for the HTTP trigger to use
- [x] **HTTP trigger auth** — optional `auth` section on HTTP trigger sources (`type: basic`, `user`/`password` from `$secrets`). Runtime validates the Authorization header; omit for no auth
